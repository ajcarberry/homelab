...
# EC2 Deployment
# =================================
resource "aws_instance" "sonarr_ec2" {
  count                       = "${var.instance_count}"
  ami                         = "${data.aws_ami.sonarr.id}"
  ...
  tags {
    ...
    OS            = "ubuntu"
    Base_AMI_Name = "${data.aws_ami.sonarr.name}"
  }
...
resource "aws_route53_record" "a_record" {
  ...
  type      = "CNAME"
  ttl       = "300"
  records   = ["${element(aws_instance.sonarr_ec2.*.public_dns, count.index)}"]
}
